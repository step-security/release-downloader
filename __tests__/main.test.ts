...
beforeEach(() => {
  ...
  nock('https://api.github.com')
    .get('/repos/step-security/foo-app/releases/tags/1.0.0')
    .reply(200, readFromFile('3-empty-assets.json'))

  nock('https://api.github.com', {
    reqheaders: { accept: 'application/octet-stream' }
  })
    .get('/repos/step-security/setup-terraform/releases/assets/66946546')
    .replyWithFile(200, `${__dirname}/resource/assets/test-1.txt`)

  nock('https://api.github.com', {
    reqheaders: { accept: 'application/octet-stream' }
  })
    .get('/repos/step-security/setup-terraform/releases/assets/66946547')
    .replyWithFile(200, `${__dirname}/resource/assets/test-2.txt`)

  nock('https://api.github.com', {
    reqheaders: { accept: 'application/octet-stream' }
  })
    .get('/repos/step-security/setup-terraform/releases/assets/66946548')
    .replyWithFile(200, `${__dirname}/resource/assets/3-test.txt`)

  nock('https://api.github.com', {
    reqheaders: { accept: 'application/octet-stream' }
  })
    .get('/repos/step-security/setup-terraform/releases/assets/66946549')
    .replyWithFile(200, `${__dirname}/resource/assets/downloader-test.pdf`)

  nock('https://api.github.com', {
    reqheaders: { accept: 'application/octet-stream' }
  })
    .get('/repos/step-security/setup-terraform/releases/assets/66946550')
    .replyWithFile(200, `${__dirname}/resource/assets/lorem-ipsum.pdf`)

  nock('https://api.github.com', {
    reqheaders: { accept: 'application/octet-stream' }
  })
    .get('/repos/step-security/setup-terraform/releases/assets/66946552')
    .replyWithFile(200, `${__dirname}/resource/assets/archive-example.zip`)

  nock('https://api.github.com', {
    reqheaders: { accept: 'application/octet-stream' }
  })
    .get('/repos/step-security/setup-terraform/releases/assets/66946551')
  ...
  nock('https://api.github.com/')
    .get('/repos/step-security/slick-pg/releases')
    .reply(200, readFromFile('4-with-prerelease.json'))

  nock('https://api.github.com', {
    reqheaders: { accept: 'application/octet-stream' }
  })
    .get('/repos/step-security/slick-pg/releases/assets/66946546')
  ...
  nock('https://api.github.com')
    .get('/repos/step-security/tar-zip-ball-only-repo/releases/latest')
    .reply(200, readFromFile('6-tar-zip-ball-only-repo.json'))

  nock('https://api.github.com', {
    reqheaders: { accept: '*/*' }
  })
    .get('/repos/step-security/tar-zip-ball-only-repo/tarball/1.0.0')
    .replyWithFile(
      200,
  ...
  nock('https://api.github.com', {
    ...
    .get('/repos/step-security/tar-zip-ball-only-repo/zipball/1.0.0')
    .replyWithFile(
      200,
...
test('Fail when a release with no assets are obtained', async () => {
  const downloadSettings: IReleaseDownloadSettings = {
    sourceRepoPath: 'step-security/foo-app',
    isLatest: false,
    preRelease: false,
    tag: '1.0.0',
    id: '',
...
test('Download from latest prerelease', async () => {
  const downloadSettings: IReleaseDownloadSettings = {
    sourceRepoPath: 'step-security/slick-pg',
    isLatest: true,
    preRelease: true,
    tag: '',
    id: '',
...
test('Download from a release containing only tarBall & zipBall', async () => {
  const downloadSettings: IReleaseDownloadSettings = {
    sourceRepoPath: 'step-security/tar-zip-ball-only-repo',
    isLatest: true,
    preRelease: false,
    tag: '',
    id: '',
